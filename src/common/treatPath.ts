...
export const treatPath = async (operation: IHttpOperation<false>): Promise<string> => {
  ...
  if (PATHVARREGEXP.test(path)) {
    const pathVariables = operation.request?.path
    if (pathVariables != null) {
      for (let i = 0; i < pathVariables.length; i++) {
        const example = operation.request?.path?.[i]?.examples?.[0]
        const altExample = operation.request?.path?.[i]?.schema?.examples as Array<string | number | boolean>
